...
fn get_loc() -> u64 {
	let output = Command::new("tokei").args(["--output", "json"]).output().expect("Failed to run tokei. Is it installed?");

	if !output.status.success() {
		panic!("tokei failed: {}", String::from_utf8_lossy(&output.stderr));
	}

	let json: serde_json::Value = serde_json::from_slice(&output.stdout).expect("Failed to parse tokei output");

	json["Total"]["code"].as_u64().expect("Failed to get LOC from tokei output")
}
...
fn gist_file_exists(gist_id: &str, filename: &str) -> Result<bool, String> {
	...
	let gist: Gist = serde_json::from_slice(&output.stdout).map_err(|e| format!("Failed to parse gist: {}", e))?;
...
fn create_gist_file(gist_id: &str, filename: &str, loc: u64) -> Result<(), String> {
	...
	let output = Command::new("gh")
		.args(["api", "--method", "PATCH", &format!("gists/{}", gist_id), "--input", "-"])
		.stdin(Stdio::piped())
		.stdout(Stdio::piped())
		.stderr(Stdio::piped())
		.spawn()
...

	{
		...
		stdin.write_all(body.to_string().as_bytes()).map_err(|e| format!("Failed to write to stdin: {}", e))?;
	}

	let output = output.wait_with_output().map_err(|e| format!("Failed to wait for gh: {}", e))?;

	if !output.status.success() {
		return Err(format!("gh api PATCH failed: {}", String::from_utf8_lossy(&output.stderr)));
	}
