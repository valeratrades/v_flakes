...
- `github`: two new labels, `t:feature` (`#2bd4a0`) and `t:research` (`#5f4b8b`). `t:chore` → `t:feature` → `t:enhancement` → `t:research` is now one ordered axis — how much of the end result is known when the issue is filed — and the descriptions say so: fully specified / end result known but the work isn't small / a direction whose end shape is decided along the way / deliverable is an answer rather than a change. `t:bug` and `t:question` are unchanged and sit off that axis. `c:*` still carries what would otherwise be `t:refactor` / `t:perf` / `t:docs`.
...
- `readme_fw`: new `ste` arg (default `false`) — the ASD-STE100 check over `usage` / `installation*` is opt-in. `curl` + `cargo-binstall` are only added to the shell when it is on.

- `readme_fw`: a section source that exists but holds no non-comment content is now treated as the user opting out — the section is dropped from the README instead of rendering empty. Comment syntax is per-extension (`<!-- -->` for md, `//` + `/* */` for typ, `#` for sh). Missing files still get an auto-generated `TODO` (and still render); only emptying one out opts out. Blank installation files also stop counting toward the multi-install `<h3>` layout.
- `utils.combine`: **bugfix** — the Determinate Nix guard probed a bare `nix`, but `nix` is not on the devShell's own PATH (mkShell does not inherit the user profile). It only ever passed on NixOS, where `/run/current-system/sw/bin` leaks in; against a profile-based install `nix config show lazy-trees` produced nothing, which compared unequal to `true` and hard-failed **every** shell — including on machines actually running Determinate Nix with `lazy-trees = true` set. Now searches `$HOME/.nix-profile/bin`, `/nix/var/nix/profiles/default/bin` and `/run/current-system/sw/bin` before judging, and warns (instead of exiting) when no nix can be found at all, since unverifiable is not the same as violated.
- `py`: re-exports `python` — the interpreter it provisions (default `pkgs.python312`, overridable via the `python` arg) is now readable back off the module. Build `packages.default` and any version string off `py.python` / `py.python.pythonVersion` instead of naming `python312` / `"3.12"` in the consumer flake: the devShell and the package can then no longer drift onto different interpreters, and the version moves for the whole fleet from one place here.
