...
//!
//! Usage:
//!   bump-crate.rs --crate "name:mode" [--crate "name:mode" ...] --version-var-postfix "Postfix"
//!
//! Modes:
...
//!
//! Examples:
//!   bump-crate.rs --crate "tracey:binstall" --crate "codestyle:binstall" --version-var-postfix "Version"
//!   bump-crate.rs --crate "tracey:source" --version-var-postfix "Version"
...
fn update_src_hash(content: &str, crate_name: &str, new_hash: &str) -> String {
	...
	let pattern = format!(r#"(pname = "{}";\s+version = [^;]+;\s+hash = ")[^"]+(")"#, regex::escape(crate_name));
	let re = Regex::new(&pattern).unwrap();
...
fn get_cargo_hash_from_build_error(crate_name: &str, version: &str, src_hash: &str, repo_root: &Path) -> Option<String> {
	...
	let output = Command::new("nix-build").args(["--impure", "--expr", &nix_expr]).current_dir(repo_root).output().ok()?;
...
fn to_partial_semver(version: &str) -> String {
	...
	if parts.len() >= 2 { format!("{}.{}", parts[0], parts[1]) } else { version.to_string() }
}
...
fn bump_crate_binstall(crate_cfg: &CrateConfig, repo_root: &Path) -> Result<bool, String> {
	...
	let flake_content = fs::read_to_string(&flake_path).map_err(|e| format!("Failed to read flake.nix: {}", e))?;
	let rs_content = fs::read_to_string(&rs_path).map_err(|e| format!("Failed to read rs/default.nix: {}", e))?;

	let current = get_current_version(&flake_content, &crate_cfg.version_var).ok_or_else(|| format!("Could not find {} in flake.nix", crate_cfg.version_var))?;
	...
	fs::write(&flake_path, &flake_content).map_err(|e| format!("Failed to write flake.nix: {}", e))?;
	fs::write(&rs_path, &rs_content).map_err(|e| format!("Failed to write rs/default.nix: {}", e))?;
...
fn print_usage() {
	eprintln!("Usage: bump-crate.rs --crate \"name:mode\" [--crate ...] --version-var-postfix \"Postfix\"");
	eprintln!();
	eprintln!("Modes:");
	...
	eprintln!();
	eprintln!("Examples:");
	eprintln!("  bump-crate.rs --crate \"tracey:binstall\" --crate \"codestyle:binstall\" --version-var-postfix \"Version\"");
	eprintln!("  bump-crate.rs --crate \"tracey:source\" --version-var-postfix \"Version\"");
}
