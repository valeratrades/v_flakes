  ...
  let
    ...
    importFile = installConfig: jobSpec:
      let
        ...
        valueWithHookPre =
          if builtins.hasAttr jobName hookPre
          then
            let
        ...
        ldLibPathSetup = builtins.concatStringsSep "" (map
          (pkg:
    ...
    containerReleaseWorkflow =
      if containerRelease != null then
    ...
    syncForkWorkflow =
      if syncFork then
        ...
        in
        (pkgs.formats.yaml { }).generate "" (builtins.removeAttrs syncSpec [ "standalone" ])
      else null;
